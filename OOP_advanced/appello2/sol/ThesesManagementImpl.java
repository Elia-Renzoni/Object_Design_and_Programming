...
public class ThesesManagementImpl implements ThesesManagement {
    ...
    public void registerThesis(int thesisId, String title, int studentId) {
        ...
        check(info.getSecond().isPresent(),()->new IllegalArgumentException("student already has a thesis"));
        this.theses.put(thesisId, studentId);
        info.setY(Optional.of(new Thesis(title)));
    }
    
    private Thesis thesisById(int id){
        return this.students.get(this.theses.get(id)).getSecond().get();
    }
    ...
    public Map<String, Status> statusByStudent() {
        return this.students
                   .values()
                   .stream()
                   .filter(e -> e.getSecond().isPresent())
                   .map(e -> new Pair<>(e.getFirst(),e.getSecond().get()))
                   .collect(Collectors.toMap(e -> e.getFirst(), e -> e.getSecond().getStatus()));
    }
